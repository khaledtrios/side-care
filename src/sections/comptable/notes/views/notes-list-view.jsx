...
export default function NotesListView() {
  ...
  const handleViewRow = useCallback(
    (id) => {
      router.push(paths.comptable.notes.view(id));
    },
    [router]
  );
  return (
    <>
      <ComptableContent>
          ...
          action={
            <Stack flexDirection="row" spacing={1} sx={{ display: { xs: 'none', md: 'flex' } }}>
              <Button
                ...
                variant="contained"
                color="primary"
                href={paths.comptable.notes.add}
                LinkComponent={RouterLink}
              >
                Ajouter
              </Button>

              <Button onClick={() => mail.onTrue()} startIcon={<Iconify icon="mynaui:send-solid" />} variant="outlined" color="primary">
                Relance Email
              </Button>

              <Button variant="contained" startIcon={<Iconify icon="mdi:file-excel" />} onClick={() => {}}>
                Détail notes de frais (.xlsx)
              </Button>

              <Button variant="outlined" startIcon={<Iconify icon="mdi:folder-zip" />} onClick={() => {}}>
                Justificatifs (.zip)
              </Button>
            </Stack>
          }
        ...
        <Box sx={{ display: { xs: 'flex', md: 'none' }, flexDirection: 'column', gap: 1, mt: 2, mb: { xs: 3, md: 0 } }}>
          <Button startIcon={<Iconify icon="mingcute:add-line" />} variant="contained" color="primary" href={paths.comptable.notes.add} LinkComponent={RouterLink} sx={{ width: '100%' }}>
            Ajouter
          </Button>

          <Button onClick={() => mail.onTrue()} startIcon={<Iconify icon="mynaui:send-solid" />} variant="outlined" color="primary" sx={{ width: '100%' }}>
            Relance Email
          </Button>

          <Button variant="contained" startIcon={<Iconify icon="mdi:file-excel" />} sx={{ width: '100%' }} onClick={() => {}}>
            Détail notes de frais (.xlsx)
          </Button>

          <Button variant="outlined" startIcon={<Iconify icon="mdi:folder-zip" />} sx={{ width: '100%' }} onClick={() => {}}>
            Justificatifs (.zip)
          </Button>
        </Box>

        <Card>
          <NotesTableToolbar filters={filters} onResetPage={table.onResetPage} dateError={dateError} options={{ status: statusList }} />

          {canReset && (
            <NotesTableFilterResult filters={filters} totalResults={dataFiltered.length} onResetPage={table.onResetPage} sx={{ p: 2.5, pt: 0 }} />
          )}
          ...
          <Box sx={{ position: 'relative' }}>
            ...
            <Scrollbar sx={{ minHeight: 444 }}>
              <Table size={table.dense ? 'small' : 'medium'} sx={{ minWidth: 960 }}>
                <TableHeadCustom order={table.order} orderBy={table.orderBy} headLabel={TABLE_HEAD} rowCount={dataFiltered.length} onSort={table.onSort} />
                ...
                <TableBody>
                  ...
                  <TableEmptyRows height={table.dense ? 56 : 56 + 20} emptyRows={emptyRows(table.page, table.rowsPerPage, dataFiltered.length)} />
