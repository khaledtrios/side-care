...
export function BankingOverview({ sx, ...other }) {
  ...
  const chartColors =
    tabs.value === 'income' ? [theme.palette.success.dark] : [theme.palette.warning.dark];

  const chartOptions = useChart({
    colors: chartColors,
    xaxis: { categories: ['Jan', 'Fév', 'Mar', 'Avr', 'Mai', 'Juin', 'Juil', 'Aoû', 'Sep'] },
    stroke: { width: 3 },
    tooltip: {
  ...
  const renderBalance = (
    <Box sx={{ flexGrow: 1 }}>
      ...
      >
        Solde total
        <Tooltip title="Informations sur le solde">
