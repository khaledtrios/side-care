              ...
              <Tooltip key={`${employee.id}-${day.dayNumber}`} title={day.formattedDate}>
                ...
                >
                  <Typography
                    variant="caption"
                    fontWeight="bold"
                    sx={{ color: isHighlighted ? 'primary.contrastText' : 'text.secondary', lineHeight: 1 }}
                  >
                    {day.dayLabel}
                  </Typography>
                  <Typography
                    variant="caption"
                    sx={{ color: isHighlighted ? 'primary.contrastText' : 'text.primary', lineHeight: 1.4 }}
                  >
                    {day.dayNumber}
                  </Typography>
                </Box>
...
export default function PlanWeek({ employees, startDate }) {
  const theme = useTheme();
  ...
  const { groupedByCompany } = useMemo(() => {
    const processed = employees.map((emp) => ({
      ...emp,
      startDateObj: emp.startDate ? new Date(emp.startDate) : new Date(0),
      endDateObj: emp.endDate ? new Date(emp.endDate) : new Date(0),
    }));
      ...
      <Table>
        <TableHead>
          <TableRow>
            ...
            <TableCell
              sx={{ ...stickyCellStyles(SECOND_STICKY_LEFT, 2, theme.palette.background.paper) }}
            >
              Jour travaillé
            </TableCell>
